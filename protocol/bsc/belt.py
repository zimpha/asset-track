...
class Belt(object):
    ...
    def __init__(self, vault_abi, swap_abi, master_abi):
        ...
        self.stable_swap = w3.eth.contract(abi=swap_abi, address=self.StableSwapB)
        self.master_belt = w3.eth.contract(abi=master_abi, address=self.MasterBelt)
        self.farm_info = None
    ...
    def staked(self, user, pool_name, block_number='latest', optimizer=None):
        ...
        if optimizer is None:
            ...
            if pool_name in belt_stake_pool:
                ...
                shares += self.master_belt.functions.stakedWantTokens(pid, user).call(block_identifier=block_number)
        else:
    ...
    def _usd_staked(self, user, block_number='latest', optimizer=None):
        ...
        if optimizer is None:
            ...
            _, shares = optimizer.staked(user, pool_name, block_number=block_number)
        if shares > 0:
            token_price = self.stable_swap.functions.get_virtual_price().call(block_identifier=block_number)
